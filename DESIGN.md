...
- Every screenshot ships from `public/*.webp` with a `.webp.json` provenance sidecar and descriptive alt text. Clicking a screen (hero slideshow or the marquee) opens the image in a new tab (`target="_blank"`, `rel="noopener noreferrer"`). The looping marquee copy is `aria-hidden` and out of the tab order.
...
- Idle state is `grayscale` so their purple and pink stay out of the page's chroma. Color is driven by scroll: `--illo-gray` is 0 when the drawing's center sits in the middle fifth of the viewport and eases back to 1 toward the edges (smoothstep over about 38% of the viewport height). The same pass starts the stroke-in the first time a drawing enters that falloff.
- Dark scheme adds `invert hue-rotate-180`, which lifts the dark linework to light and keeps the purple recognizably purple once lit.
