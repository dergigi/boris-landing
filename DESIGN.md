...
- Idle state is `grayscale` so their purple and pink stay out of the page's chroma. An IntersectionObserver marks one `is-lit` while at least 40% of it is on screen; that releases the color (300ms filter transition) on both touch and pointer. Off-screen drawings go back to gray.
